...
start_llm() {
  ...
  start_one graphmem-v5-llm "${LLM_GPUS}" "${STATE_DIR}/llm.log" \
    "'${LLM_MODEL_PATH}' --served-model-name '${LLM_SERVED_NAME}' --host 127.0.0.1 --port 8002 --tensor-parallel-size 2 --max-model-len '${LLM_MAX_MODEL_LEN}' --gpu-memory-utilization 0.88 --max-num-seqs 128 --max-num-batched-tokens 65536 --trust-remote-code"
  wait_one graphmem-v5-llm 8002 "${STATE_DIR}/llm.log"
}

stop_services() {
  local session
  for session in graphmem-v5-embed-heartbeat graphmem-v5-embed graphmem-v5-llm; do
    if tmux has-session -t "${session}" 2>/dev/null; then
...
case "${1:-}" in
  ...
  *) echo "usage: $0 {start|start-embed|start-llm|start-heartbeat|stop|status}" >&2; exit 2 ;;
esac
